...
interface ProductBlockProps {
  ...
  onChange?: (questions: ProductQuestion[]) => void;
  readOnly?: boolean;
...
const defaultProductQuestions: Omit<ProductQuestion, "id">[] = [
  {
    type: "text",
    title: "Product Name",
    description: "Enter the full product name as appears on the product or packaging",
    required: true,
    options: [],
  },
  {
    type: "text",
    title: "Product Model/Type",
    description: "Model number or type designation",
    required: true,
    options: [],
  },
  {
    ...
    options: [
      ...
      { id: "cosmetics", label: "Cosmetics", value: "cosmetics" },
      { id: "textiles", label: "Textiles", value: "textiles" },
      { id: "other", label: "Other", value: "other" },
    ],
  },
  {
    type: "boolean",
    title: "Is CE marking present?",
    description: "Check if the CE marking is visible and legible on the product",
    required: true,
    options: [],
  },
  {
    type: "photo",
    title: "Product Photo",
    description: "Take a clear photo of the product",
    required: true,
    options: [],
  },
];

export function ProductBlock({ questions, onChange, readOnly = false, legislations = [], validationErrors = {}, blockId = "" }: ProductBlockProps) {
  const [localQuestions, setLocalQuestions] = useState<ProductQuestion[]>(
    questions || defaultProductQuestions.map((q, index) => ({ ...q, id: `product_q_${index}` }))
  );
  const [movedQuestionId, setMovedQuestionId] = useState<string | null>(null);

  const handleQuestionChange = (index: number, updates: Partial<ProductQuestion>) => {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h4 className="font-medium text-sm text-gray-700">Product Block</h4>
          <p className="text-xs text-gray-500">Questions about the product itself</p>
        </div>
        {!readOnly && (
      ...
      {localQuestions.map((question, index) => (
        <div key={question.id} className={`border-l-4 border-blue-300 pl-4 space-y-3 bg-blue-50/50 p-3 rounded-r transition-all duration-300 ease-in-out ${movedQuestionId === question.id ? 'animate-swap' : ''}`}>
          <div className="flex items-center justify-between">
          ...
          <div className="space-y-2 pt-2">
            <div className="flex items-center justify-between">
              <Label htmlFor={`photos-allowed-product-${index}`} className="text-xs">
                Photo(s) Allowed
              </Label>
              <Switch
                id={`photos-allowed-product-${index}`}
            ...
            <div className="flex items-center justify-between">
              <Label htmlFor={`files-allowed-product-${index}`} className="text-xs">
                File(s) Allowed
              </Label>
              <Switch
                id={`files-allowed-product-${index}`}
